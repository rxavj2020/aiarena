...
export async function TenantAdminWorkspace({ tenant, firestoreConnected }: { tenant: Tenant; firestoreConnected: boolean }) {
  ...
  return <div className="mx-auto max-w-[1200px] space-y-6">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full bg-[#f3eadb] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-[#9b682d]"><Store className="h-3.5 w-3.5" /> Store admin</div><h1 className="font-display text-3xl font-bold tracking-tight">{tenant.name}</h1><p className="mt-1 text-sm text-black/50">Manage your products and orders from one private control panel.</p></div><div className="flex flex-wrap gap-2"><Link href="/dashboard?tab=settings" aria-label="Open store settings" className="inline-flex items-center gap-2 rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm font-bold transition hover:border-black/30"><Settings2 className="h-4 w-4" /> Settings</Link><Link href="/dashboard?tab=plugins" aria-label="Open store plugins" className="inline-flex items-center gap-2 rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm font-bold transition hover:border-black/30"><Plug className="h-4 w-4" /> Plugins</Link><Link href={`/store/${tenant.slug}`} target="_blank" className="inline-flex items-center gap-2 rounded-xl bg-[#11110f] px-3.5 py-2.5 text-sm font-bold text-white transition hover:bg-black"><ExternalLink className="h-4 w-4" /> View store</Link></div></div>
