...
import { ArrowRight, Check, ChevronRight, Cloud, Code2, ExternalLink, Globe2, KeyRound, Loader2, Palette, Rocket, Save, ShieldCheck, Store } from "lucide-react";
import { connectWorkspaceDomain, connectWorkspaceFirestore, launchWorkspace, saveWorkspaceBranding, selectWorkspace, verifyWorkspaceDomain, type FirestoreInput } from "@/actions/platform";
...
export function WorkspaceSetup({ tenant: initialTenant, setup: initialSetup }: { tenant: Tenant; setup: Setup }) {
  const router = useRouter();
  const [tenant, setTenant] = useState(initialTenant);
  const [setup, setSetup] = useState(initialSetup);
  const [tab, setTab] = useState<"overview" | "data" | "brand" | "domain">("overview");
  ...
  const [firestore, setFirestore] = useState<FirestoreInput>({ projectId: "", databaseId: "(default)", clientEmail: "", privateKey: "", collectionPrefix: "store_", autoSyncMinutes: 0 });
  ...
  return (
    <div className="space-y-7">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
        <div><Link href="/platform" className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-black/45 hover:text-black"><ChevronRight className="h-4 w-4 rotate-180" /> All workspaces</Link><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl text-lg font-bold" style={{ backgroundColor: `${tenant.accentColor}20`, color: tenant.accentColor }}>{tenant.name.charAt(0).toUpperCase()}</div><div><div className="flex flex-wrap items-center gap-2"><h1 className="font-display text-3xl font-bold tracking-tight">{tenant.name}</h1><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${tenant.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{tenant.status === "active" ? "Live" : "Setup"}</span></div><p className="mt-1 text-sm text-black/45">{tenant.slug}.aurelia.app · {planMeta[tenant.plan].name} plan</p></div></div></div>
        <div className="flex flex-wrap gap-2"><Link href={`/site/${tenant.slug}`} target="_blank" className="inline-flex items-center gap-2 rounded-xl border border-black/10 bg-white px-4 py-2.5 text-sm font-bold transition hover:border-black/30"><ExternalLink className="h-4 w-4" /> Preview site</Link><form action={selectWorkspace.bind(null, tenant.id, "/admin")}><button className="inline-flex items-center gap-2 rounded-xl bg-[#11110f] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-black"><Store className="h-4 w-4" /> Open admin</button></form></div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
        <div className="rounded-[24px] border border-black/10 bg-white p-5 sm:p-7">
          ...
          {tab === "data" ? <FirestorePanel values={firestore} setValues={setFirestore} onConnect={connectFirestore} busy={busy} connected={setup.firestoreConnected} /> : null}
        ...
        <aside className="space-y-4"><div className="rounded-[24px] bg-[#11110f] p-6 text-white"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#e9c78d] text-[#11110f]"><ShieldCheck className="h-5 w-5" /></div><h3 className="mt-5 font-display text-xl font-bold">Safe by default</h3><p className="mt-2 text-sm leading-6 text-white/50">Your Firestore service account is encrypted before it is stored. It is only used by this workspace and never returned to the browser.</p><div className="mt-5 flex items-center gap-2 text-xs font-semibold text-[#e9c78d]"><KeyRound className="h-3.5 w-3.5" /> Tenant-scoped credentials</div></div><div className="rounded-[24px] border border-black/10 bg-white p-6"><div className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/35">Public URL</div><Link href={`/site/${tenant.slug}`} target="_blank" className="mt-3 block truncate text-sm font-bold hover:underline">aurelia.app/site/{tenant.slug} <ExternalLink className="ml-1 inline h-3.5 w-3.5" /></Link><p className="mt-2 text-xs leading-5 text-black/45">Connect a custom domain when the site is ready for customers.</p></div></aside>
      </div>
    </div>
  );
}
...
function FirestorePanel({ values, setValues, onConnect, busy, connected }: { values: FirestoreInput; setValues: (v: FirestoreInput) => void; onConnect: () => void; busy: boolean; connected: boolean }) {
  return <div className="mt-8"><PanelHeading icon={<Cloud className="h-5 w-5" />} title="Connect Firestore" description="Use a Firebase service account. We test the connection before enabling launch and encrypt the credentials at rest." /><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Google Cloud project ID"><input value={values.projectId} onChange={(e) => setValues({ ...values, projectId: e.target.value })} placeholder="my-commerce-project" className="field" /></Field><Field label="Firestore database ID"><input value={values.databaseId} onChange={(e) => setValues({ ...values, databaseId: e.target.value })} placeholder="(default)" className="field" /></Field><Field label="Service account client email" wide><input value={values.clientEmail} onChange={(e) => setValues({ ...values, clientEmail: e.target.value })} placeholder="firebase-adminsdk@project.iam.gserviceaccount.com" className="field" autoComplete="off" /></Field><Field label="Private key" wide><textarea value={values.privateKey} onChange={(e) => setValues({ ...values, privateKey: e.target.value })} placeholder="-----BEGIN PRIVATE KEY-----\n..." rows={5} className="field resize-y font-mono text-xs" autoComplete="off" /></Field><Field label="Collection prefix"><input value={values.collectionPrefix} onChange={(e) => setValues({ ...values, collectionPrefix: e.target.value })} className="field" /></Field><Field label="Auto-sync interval"><select value={values.autoSyncMinutes} onChange={(e) => setValues({ ...values, autoSyncMinutes: Number(e.target.value) })} className="field"><option value={0}>Manual sync</option><option value={15}>Every 15 minutes</option><option value={60}>Every hour</option><option value={360}>Every 6 hours</option></select></Field></div><div className="mt-5 flex items-start gap-2 rounded-xl bg-[#eef3ff] px-3.5 py-3 text-xs leading-5 text-[#536dce]"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /> Create a least-privilege service account for this workspace. The test writes only a temporary connection marker to Firestore.</div><ActionButton onClick={onConnect} busy={busy} icon={connected ? <Check className="h-4 w-4" /> : <Cloud className="h-4 w-4" />}>{connected ? "Re-test & save connection" : "Test & connect Firestore"}</ActionButton></div>;
}
...
function DomainPanel({ domain, setDomain, current, onSave, onVerify, busy }: { domain: string; setDomain: (v: string) => void; current: Setup["domain"]; onSave: () => void; onVerify: () => void; busy: boolean }) {
  return <div className="mt-8"><PanelHeading icon={<Globe2 className="h-5 w-5" />} title="Connect a custom domain" description="Customers can use your own domain instead of the Aurelia URL." />{current ? <div className="mt-6 rounded-2xl border border-black/10 bg-[#fafaf8] p-4"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-bold">{current.hostname}</div><div className="mt-1 flex items-center gap-2 text-xs text-black/45"><span className={`h-1.5 w-1.5 rounded-full ${current.status === "verified" ? "bg-emerald-500" : "bg-amber-500"}`} />{current.status === "verified" ? "Verified and serving" : "Awaiting DNS verification"}</div></div><span className="rounded-full bg-amber-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-700">{current.status}</span></div>{current.status === "pending" ? <div className="mt-4 border-t border-black/10 pt-4 text-xs leading-5 text-black/55"><div className="font-bold text-black/70">Add this DNS record at your provider</div><div className="mt-2 grid gap-2 sm:grid-cols-3"><div><span className="text-black/35">Type</span><div className="font-mono font-bold">CNAME</div></div><div><span className="text-black/35">Name</span><div className="font-mono font-bold">@</div></div><div><span className="text-black/35">Target</span><div className="truncate font-mono font-bold">aurelia.app</div></div></div><p className="mt-3 text-black/40">Verification will be enabled when DNS points to Aurelia. SSL is provisioned automatically.</p></div> : null}</div> : null}<div className="mt-6"><Field label="Your domain"><input value={domain} onChange={(e) => setDomain(e.target.value)} placeholder="shop.yourbrand.com" className="field" /><p className="mt-1.5 text-[11px] text-black/40">Enter the hostname only; we’ll provide the DNS steps after saving.</p></Field></div><div className="flex flex-wrap items-center gap-3"><ActionButton onClick={onSave} busy={busy} icon={<Globe2 className="h-4 w-4" />}>Save domain</ActionButton>{current?.status === "pending" ? <button type="button" onClick={onVerify} disabled={busy} className="mt-7 inline-flex items-center gap-2 rounded-xl border border-black/10 px-4 py-3 text-sm font-bold transition hover:border-black/30 disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />} Check DNS</button> : null}</div></div>;
}
...
function Field({ label, wide, children }: { label: string; wide?: boolean; children: React.ReactNode }) { return <label className={`block ${wide ? "sm:col-span-2" : ""}`}><span className="mb-1.5 block text-xs font-bold text-black/65">{label}</span>{children}</label>; }
