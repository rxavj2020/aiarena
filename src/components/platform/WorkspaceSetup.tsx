...
export function WorkspaceSetup({ tenant: initialTenant, setup: initialSetup, oauthConfigured }: { tenant: Tenant; setup: Setup; oauthConfigured: boolean }) {
    ...
    <div className="space-y-7">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
        <div className="rounded-[24px] border border-black/10 bg-white p-5 sm:p-7">
          ...
          {tab === "data" ? <FirestorePanel tenantId={tenant.id} tenantSlug={tenant.slug} values={firestore} setValues={setFirestore} onConnect={connectFirestore} busy={busy} connected={setup.firestoreConnected} oauthConnected={setup.firestoreAuthMode === "oauth"} oauthConfigured={oauthConfigured} /> : null}
...
function FirestorePanel({ tenantId, tenantSlug, values, setValues, onConnect, busy, connected, oauthConnected, oauthConfigured }: { tenantId: string; tenantSlug: string; values: FirestoreInput; setValues: (v: FirestoreInput) => void; onConnect: () => void; busy: boolean; connected: boolean; oauthConnected: boolean; oauthConfigured: boolean }) {
  return <div className="mt-8"><PanelHeading icon={<Cloud className="h-5 w-5" />} title="Connect Firestore" description="Use Google OAuth or a Firebase service account. We test read and write access before enabling launch and encrypt credentials at rest." />
    {oauthConnected ? <div className="mt-6 flex items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3.5 py-3 text-xs leading-5 text-emerald-800"><Check className="mt-0.5 h-4 w-4 shrink-0" /> Google OAuth is connected for this workspace. Reconnect below to change the authorized Google account.</div> : null}
    <div className="mt-7 grid gap-5 sm:grid-cols-2">
      <Field label="Google Cloud project ID" help="Find this in Firebase Project settings. It must be the project where this workspace's Firestore database lives."><input value={values.projectId} onChange={(e) => setValues({ ...values, projectId: e.target.value })} placeholder="my-commerce-project" className="field" /></Field>
      ...
      <Field label="Collection prefix" help="A workspace slug is added automatically so tenants never share collections."><input value={values.collectionPrefix} onChange={(e) => setValues({ ...values, collectionPrefix: e.target.value })} className="field" /></Field>
    </div>
    <div className="mt-5 flex items-start gap-2 rounded-xl bg-[#eef3ff] px-3.5 py-3 text-xs leading-5 text-[#536dce]"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /> Google OAuth grants access to the account you choose. The manual path uses a least-privilege service account. In both cases the test writes only a temporary connection marker to this workspace&apos;s namespaced Firestore collection.</div>
    <div className="flex flex-wrap gap-3">
      ...
      {oauthConfigured ? <form action={connectWorkspaceFirestoreOAuth.bind(null, tenantId, `/platform/stores/${tenantSlug}`)}><input type="hidden" name="projectId" value={values.projectId} /><button type="submit" disabled={busy || !values.projectId} className="mt-7 inline-flex items-center gap-2 rounded-xl border border-[#536dce]/25 bg-[#eef3ff] px-4 py-3 text-sm font-bold text-[#536dce] transition hover:border-[#536dce]/50 disabled:cursor-not-allowed disabled:opacity-45"><span className="font-black">G</span> {oauthConnected ? "Reconnect with Google" : "Connect with Google OAuth"}</button></form> : <p className="mt-7 text-xs text-black/40">Google OAuth is not configured on this deployment. Use the manual service-account path or ask the platform administrator to add Google OAuth credentials.</p>}
    </div>
  </div>;
}
