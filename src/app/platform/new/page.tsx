...
export default function NewWorkspacePage() {
  ...
  return (
    <div className="mx-auto max-w-[1040px]">
      ...
      <div className="mb-9 max-w-2xl"><div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[#f3eadb] px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-[#9b682d]"><Sparkles className="h-3.5 w-3.5" /> New workspace</div><h1 className="font-display text-4xl font-bold leading-[1.04] tracking-tight sm:text-5xl">Start with your brand.<br /><span className="text-black/35">Connect data next.</span></h1><p className="mt-4 max-w-xl text-sm leading-6 text-black/55 sm:text-base">Your workspace is the private home for your admin tools, data connection and public storefront. No demo products will be created here.</p></div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
        <section className="rounded-[24px] border border-black/10 bg-white p-5 sm:p-7">
          ...
          <div className="mt-7 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2 text-[11px] text-black/40"><LockKeyhole className="h-3.5 w-3.5" /> Your workspace is private by default</div><button type="button" onClick={submit} disabled={pending || !name.trim()} className="inline-flex items-center gap-2 rounded-xl bg-[#11110f] px-5 py-3 text-sm font-bold text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-40">{pending ? "Creating…" : "Create & connect data"}<ArrowRight className="h-4 w-4" /></button></div>
        </section>
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-[24px] border border-black/10 bg-white p-6"><div className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/35">What you get</div><ul className="mt-4 space-y-3 text-sm text-black/65"><li className="flex gap-3"><span className="mt-0.5 text-emerald-600">✓</span> A private admin workspace</li><li className="flex gap-3"><span className="mt-0.5 text-emerald-600">✓</span> A public store URL</li><li className="flex gap-3"><span className="mt-0.5 text-emerald-600">✓</span> Brand and domain controls</li><li className="flex gap-3"><span className="mt-0.5 text-emerald-600">✓</span> Plugin-ready infrastructure</li></ul></div>
        </aside>
      </div>
    </div>
  );
