    ...
    <div className="min-h-screen bg-[#11110f] px-4 py-8 text-white sm:flex sm:items-center sm:justify-center">
      <div className="grid w-full max-w-[960px] overflow-hidden rounded-[28px] border border-white/10 bg-[#191916] shadow-2xl sm:grid-cols-2">
        <div className="hidden flex-col justify-between bg-[#e9c78d] p-10 text-[#11110f] sm:flex">
          ...
          <div><h1 className="font-display text-4xl font-bold leading-[1.03] tracking-tight">Build the store your brand deserves.</h1><p className="mt-5 max-w-sm text-sm leading-6 text-[#11110f]/70">One secure workspace for your catalogue, customers, domains and the public site your customers see.</p></div>
        ...
        <div className="p-7 sm:p-10">
          ...
          <div className="mb-7"><div className="mb-3 flex h-10 w-10 items-center justify-center rounded-2xl bg-white/10 text-[#e9c78d]"><LockKeyhole className="h-5 w-5" /></div><h1 className="font-display text-2xl font-bold">Welcome back</h1><p className="mt-1 text-sm text-white/50">Sign in to manage your commerce workspaces.</p></div>
          <form action={action} className="space-y-4">
            ...
            <button disabled={pending} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#e9c78d] px-4 py-3 text-sm font-bold text-[#11110f] transition hover:bg-[#f2d8a7] disabled:opacity-60">{pending ? "Signing in…" : "Continue to Studio"}<ArrowRight className="h-4 w-4" /></button>
          </form>
          <div className="mt-6 border-t border-white/10 pt-5 text-center text-xs text-white/45">New to Aurelia Studio? <Link href="/platform/signup" className="font-semibold text-[#e9c78d] hover:underline">Create a workspace <ArrowRight className="inline h-3 w-3" /></Link></div>
        </div>
      </div>
    </div>
  );
